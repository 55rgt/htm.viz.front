...
export function makeGradient(firstColor: string, secondColor: string, stepsInt: number, n: number) {
  //attach start value
  let hasSpun = 0;

  const val1RGB = processHEX(firstColor);
  var val2RGB = processHEX(secondColor);
  ...
  for (let i = 0; i < stepsInt; i++) {
    var clampedR =
      valClampRGB[0] > 0
        ...
        : pad(
            Math.round(val1RGB[0] + (valClampRGB[0] / 100) * (stepsPerc * (i + 1))).toString(16),
            2
          );
    ...
    var clampedG =
      valClampRGB[1] > 0
        ...
        : pad(
            Math.round(val1RGB[1] + (valClampRGB[1] / 100) * (stepsPerc * (i + 1))).toString(16),
            2
          );
    ...
    var clampedB =
      valClampRGB[2] > 0
        ...
        : pad(
            Math.round(val1RGB[2] + (valClampRGB[2] / 100) * (stepsPerc * (i + 1))).toString(16),
            2
          );
...
// const result = makeGradient('#000000', '#ffffff', 100, 50);
